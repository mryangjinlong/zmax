            ...
            if(temp==null){
                ...
                if(time<=0){
                    ...
                    if(queueCount>0&&beginQueueCount>=queueCount){
                        logger.debug(name+"：已检测1分钟并且初始队列值还大等于当前队列值，说明设备已经阻塞1分钟，将设备设为不可用，并且重新开始计时");
                        temp.put("status",0);
                    }
                    temp.put("time",60000);
                }else{
                    ...
                    if(queueCount<beginQueueCount){
                        logger.debug(name+"：设备当前队列已少于1分钟起始值，说明设备正常,可以重新开始计时");
                        temp.put("time",60000);
                        temp.put("beginQueueCount",queueCount);
                        temp.put("status",1);
                    }else{
                        logger.debug(name+"：设备当前队列并不小于于1分钟初始值，说明可能存在问题，继续观察...");
                        temp.put("time",time-500);
                    }
                }
            }
