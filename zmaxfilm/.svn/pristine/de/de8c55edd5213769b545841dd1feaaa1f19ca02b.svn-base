...
(function($) {
	...
	$.fn.Spinner = function (opts) {
		...
		var defaults = { min:1, len:3, max:99}
		var options = $.extend(defaults, opts)
		var keyCodes = {up:38, down:40}
		return this.each(function() {

			var a = $(this).find('a.leftBtn'); f(a,0,"Decrease");	//加
			var c = $(this).find('a.rightBtn'); f(c,0,"Increase");	//减
			var b = $(this).find('input');f(b,1,"Amount");cv(0);	//值
